...
export default function OrdersPage() {
    ...
    // Approve/Reject specific installment
    const handleApproveInstallment = async (orderId, installmentNumber, status) => {
        try {
            ...
            if (res.ok) {
                toast.success(`Installment #${installmentNumber} marked as ${status}`);
                // Refresh data
                const updatedOrderRes = await fetch(`${BASE_URL}/orders/admin/all`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div className="relative group">
                    <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/20 to-rose-600/20 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                    <div className="relative bg-white rounded-2xl border border-slate-200/60 p-6 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500 to-rose-600 flex items-center justify-center text-white shadow-lg">
                            ...
                            {selectedOrder.manualPaymentDetails && (
                                <div className="bg-amber-50 rounded-2xl p-6 border border-amber-100 shadow-sm">
                                    <div className="flex items-center gap-2 mb-4">
                                        ...
                                        <div>
                                            <h4 className="text-sm font-bold text-amber-900 uppercase tracking-wider">Order Payment Proof</h4>
                                            <p className="text-[10px] text-amber-600 font-medium">Verify this for full payments or first installments</p>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                                        <div>
                                            <p className="text-[10px] font-bold text-amber-500 uppercase mb-1">Provider</p>
                                            <p className="font-bold text-slate-800 uppercase bg-white px-3 py-1.5 rounded-lg border border-amber-200 inline-block">{selectedOrder.manualPaymentDetails.provider}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-bold text-amber-500 uppercase mb-1">User Number</p>
                                            <p className="font-bold text-slate-800 bg-white px-3 py-1.5 rounded-lg border border-amber-200 inline-block">{selectedOrder.manualPaymentDetails.accountNumber}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-bold text-amber-500 uppercase mb-1">Transaction ID</p>
                                            <p className="font-mono font-bold text-amber-700 bg-white px-3 py-1.5 rounded-lg border border-amber-200 inline-block">{selectedOrder.manualPaymentDetails.transactionId}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-bold text-amber-500 uppercase mb-1">Time & Date</p>
                                            <div className="bg-white px-3 py-1.5 rounded-lg border border-amber-200 inline-block text-slate-800 font-bold">
                                                {selectedOrder.manualPaymentDetails.date} <span className="text-amber-500 mx-1">•</span> {selectedOrder.manualPaymentDetails.time}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        {selectedOrder.installments.map((inst, idx) => (
                                            <div key={idx} className={`p-4 rounded-xl border ${inst.status === 'completed' ? 'bg-emerald-50 border-emerald-100' : inst.status === 'pending' && inst.paymentDetails ? 'bg-amber-50 border-amber-100' : 'bg-slate-50 border-slate-100'}`}>
                                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                                    <div className="flex items-center gap-4">
                                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${inst.status === 'completed' ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-600'}`}>
                                                            {inst.installmentNumber}
                                                        </div>
                                                        <div>
                                                            <p className="font-bold text-slate-800">৳{inst.amount?.toLocaleString()}</p>
                                                            <p className="text-[10px] text-slate-500 uppercase font-medium">Due: {new Date(inst.dueDate).toLocaleDateString()}</p>
                                                        </div>
                                                    ...
                                                    {/* Payment Proof for this installment */}
                                                    {inst.paymentDetails && (
                                                        <div className="flex flex-wrap gap-2 text-[10px] bg-white/50 p-2 rounded-lg border border-slate-200/50">
                                                            <span className="font-bold uppercase text-slate-500">{inst.paymentDetails.provider}:</span>
                                                            <span className="text-slate-700">{inst.paymentDetails.accountNumber}</span>
                                                            <span className="mx-1 text-slate-300">|</span>
                                                            <span className="font-mono text-indigo-600 font-bold">{inst.paymentDetails.transactionId}</span>
                                                        </div>
                                                    )}
                                                    ...
                                                    {inst.status !== 'completed' && (
                                                        <div className="flex items-center gap-2">
                                                            <button
                                                                onClick={() => handleApproveInstallment(selectedOrder._id, inst.installmentNumber, 'completed')}
                                                            ...
                                                            <button
                                                                onClick={() => handleApproveInstallment(selectedOrder._id, inst.installmentNumber, 'failed')}
                                                                className="flex items-center gap-1 px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white rounded-lg text-[10px] font-bold transition-all"
                                                            >
                                                                <FiX size={12} /> Reject
                                                            </button>
                                                        </div>
