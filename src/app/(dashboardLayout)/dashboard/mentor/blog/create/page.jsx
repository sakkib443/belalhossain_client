        ...
        <div className="space-y-6 pb-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div className="flex gap-3">
                    <button onClick={() => setPreviewMode(!previewMode)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all ${previewMode ? 'bg-gradient-to-r from-rose-600 to-cyan-500 text-white' : isDark ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>
                    ...
                    <button onClick={() => handleSubmit('published')} disabled={loading} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-rose-600 to-cyan-500 text-white font-semibold shadow-lg disabled:opacity-50">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className={`p-6 rounded-2xl border ${isDark ? 'bg-slate-800/50 border-slate-700/50' : 'bg-white border-slate-200'}`}>
                        ...
                        <input type="text" name="title" value={formData.title} onChange={handleChange} placeholder="Enter your blog title..."
                            className={`w-full px-4 py-3 rounded-xl border text-lg font-medium ${isDark ? 'bg-slate-700/50 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'} focus:outline-none focus:ring-2 focus:ring-rose-600/20`} />
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className={`p-6 rounded-2xl border ${isDark ? 'bg-slate-800/50 border-slate-700/50' : 'bg-white border-slate-200'}`}>
                        ...
                        {formData.thumbnail ? (
                            ...
                            <label className={`flex flex-col items-center justify-center h-48 border-2 border-dashed rounded-xl cursor-pointer ${isDark ? 'border-slate-600 hover:border-rose-600' : 'border-slate-300 hover:border-rose-600'}`}>
                                <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
                                {uploading ? <div className="w-8 h-8 border-3 border-rose-600/30 border-t-rose-600 rounded-full animate-spin" /> : <><FiUpload className={isDark ? 'text-slate-500' : 'text-slate-400'} size={24} /><span className={`text-sm mt-2 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Upload</span></>}
                            </label>
                        )}
                    </div>
                    ...
                    <div className={`p-6 rounded-2xl border ${isDark ? 'bg-slate-800/50 border-slate-700/50' : 'bg-white border-slate-200'}`}>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button onClick={addTag} className="px-3 py-2 rounded-lg bg-rose-600 text-white"><FiTag size={16} /></button>
                        </div>
                        <div className="flex flex-wrap gap-2 mt-3">
                            {formData.tags.map((tag, idx) => (
                                <span key={idx} className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs ${isDark ? 'bg-rose-600/20 text-rose-500' : 'bg-rose-50 text-rose-700'}`}>
                    ...
                    <div className={`p-6 rounded-2xl border ${isDark ? 'bg-slate-800/50 border-slate-700/50' : 'bg-white border-slate-200'}`}>
                        ...
                        <label className="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="allowComments" checked={formData.allowComments} onChange={handleChange} className="w-4 h-4 rounded text-rose-600" />
