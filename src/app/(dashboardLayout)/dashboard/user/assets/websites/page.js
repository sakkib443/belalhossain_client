...
export default function MyWebsitesPage() {
    ...
    // Extract website items from orders and fetch full product details
    useEffect(() => {
        const fetchProductDetails = async () => {
            ...
            // Fetch full product details for each website
            const productsWithDetails = await Promise.all(
                websiteItems.map(async (item) => {
                    try {
                        const res = await fetch(`${API_BASE_URL}/websites/${item.product}`);
            ...
            setWebsiteProducts(productsWithDetails);
            setProductsLoading(false);
        };
    ...
    // Filter by search
    const filteredWebsites = websiteProducts.filter(w =>
        w.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        w.productDetails?.title?.toLowerCase().includes(searchTerm.toLowerCase())
    );

    // Stats
    const stats = {
        total: websiteProducts.length,
        completed: websiteProducts.filter(w => w.paymentStatus === 'completed').length,
        pending: websiteProducts.filter(w => w.paymentStatus === 'pending').length,
    };
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                            View all your purchased websites and their details
                        </p>
                    </div>
                </div>
            ...
            {filteredWebsites.length === 0 ? (
                <div className={`py-20 text-center rounded-2xl border-2 border-dashed ${isDark
                    ...
                    {!searchTerm && (
                        ...
                        >
                            Browse Websites <FiExternalLink />
                        </Link>
                    )}
                </div>
            ) : (
                                ...
                                <div className="p-5">
                                    <div className="flex flex-col lg:flex-row gap-5">
                                        ...
                                        <div className="lg:w-48 h-32 lg:h-auto rounded-xl overflow-hidden shrink-0">
                                            <img
                                                src={item.image || product?.images?.[0] || 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&q=80&w=400'}
                                                alt={item.title}
                ...
                <div className="flex items-center gap-4 text-center md:text-left">
                    ...
                    <div>
                        ...
                        <p className={`text-xs mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                            After payment confirmation, our team will contact you for website setup.
                        </p>
                    </div>
                </div>
                <Link
