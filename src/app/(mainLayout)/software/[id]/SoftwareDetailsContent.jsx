...
import {
    ...
    LuShieldCheck, LuSettings, LuFileCode, LuGlobe, LuCheck, LuSparkles, LuZap, LuX
} from "react-icons/lu";
...
export default function SoftwareDetailsContent({ initialSoftware }) {
    ...
    useEffect(() => {
        if (softwareList.length > 0 && software?._id) {
            setPopularSoftware(softwareList.filter(item => item._id !== software._id).slice(0, 3));
        }
                            ...
                            <div className="sticky top-24 space-y-5">
                                <div className="bg-white rounded-md border border-gray-200 shadow-sm overflow-hidden">
                                    <div className="relative aspect-video group cursor-pointer" onClick={() => software.videoUrl && setShowVideoModal(true)}>
                                        ...
                                        {software.videoUrl && <div className="absolute inset-0 bg-black/40 flex items-center justify-center"><MdPlayCircleOutline className="text-white text-6xl" /></div>}
                                    </div>
                                    <div className="p-5 space-y-5">
                                        ...
                                        <div className="space-y-2.5">
                                            <button onClick={handleBuyNow} className="w-full py-3 bg-teal-500 text-white font-semibold rounded-md transition-colors">Buy Now</button>
                                            <button onClick={handleAddToCart} className="w-full py-2.5 bg-white border border-gray-200 text-gray-700 font-semibold rounded-md">Add to Cart</button>
                                            {software.previewUrl && <a href={software.previewUrl} target="_blank" className="w-full py-2.5 bg-gray-50 border border-gray-200 text-gray-600 font-medium rounded-md flex items-center justify-center gap-2 text-sm"><LuExternalLink size={14} /> Live Preview</a>}
                                        </div>
                                    </div>
                                </div>
                            </div>
