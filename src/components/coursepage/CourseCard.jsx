      ...
      <div className="rounded-md border border-gray-200 bg-white p-2 text-gray-800 overflow-hidden transition-all duration-700 ease-in-out">
        ...
        <div className="mt-5 space-y-1 text-left pl-4">
          <div className="flex justify-between pr-8">
            ...
            <p className="bg-[#FD9A00] text-white text-[13px] px-2 py-1 work rounded-[4px]">
              {type}
            </p>
          </div>
        ...
        <div className="flex justify-between gap-2 items-center mt-2 mb-2">
          <Link
            href={`/${id}`}
            className="flex gap-2 text-xl items-center border bg-[#FD9A00] border-[#FD9A00] px-4 ml-2 py-2 rounded-md"
          >
          ...
          <Link
            href={`https://wa.me/8801753924093?text=${encodeURIComponent(
              `আমি "${title}" কোর্সটি করতে চাই।`
            )}`}
            target="_blank"
            rel="noopener noreferrer"
          >
            <div className="flex gap-1 text-xl items-center border border-[#FD9A00] px-4 py-2 mr-6 rounded-md hover:bg-[#e0f7f5] cursor-pointer transition">
              <FaWhatsapp className="text-xl text-[#FD9A00] font-medium" />
              <p className="text-[#FD9A00] work text-[15px] tracking-tight">
                Get Course
              </p>
            </div>
          </Link>
