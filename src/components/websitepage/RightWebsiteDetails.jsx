...
import { LuLayoutGrid, LuList, LuArrowUpDown, LuGlobe, LuStar, LuBanknote } from "react-icons/lu";
...
const RightWebsiteDetails = ({ searchQuery, selectedType }) => {
    ...
    return (
        <div className="space-y-10">
            ...
            {/* Grid display */}
            {loading ? (
                <div className={`grid gap-8 ${isGridView ? 'grid-cols-1 sm:grid-cols-2 xl:grid-cols-3' : 'grid-cols-1'}`}>
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                ...
                <div className="text-center py-32 flex flex-col items-center justify-center space-y-6">
                    ...
                    <div className="space-y-2">
                        <p className="text-2xl font-normal text-slate-800 outfit">
                            {language === 'bn' ? 'কোনো ওয়েবসাইট পাওয়া যায়নি' : 'No websites found'}
                        </p>
                        <p className="text-slate-400 text-sm work font-normal">
                            {language === 'bn' ? 'অন্য কোনো কি-ওয়ার্ড বা ফিল্টার দিয়ে চেষ্টা করুন' : 'Try different keywords or filters'}
                        </p>
                    </div>
                </div>
            )}
